...
public class DatabaseHelper {
    ...
    public void connectToDatabase() throws SQLException {
        try {
            ...
            // DEBUG FOR MESSAGES
            //printAllMessages();
            //debugPrintAllStaffAlerts();
            
            // Run this to reset the reviews table... comment again when done
            //clearAllReviews();
            
            // Run this to reset the flags table... comment again when done
            //clearAllFlags();
    ...
    public void createTables() throws SQLException {
    	...
    	// In case the table already exists, add 'resolved' if missing
    	String alterFlagsTable = "ALTER TABLE Flags ADD COLUMN IF NOT EXISTS resolved BOOLEAN DEFAULT FALSE";
    	statement.execute(alterFlagsTable);
        ...
        String alterUserTableAddBio = "ALTER TABLE cse360users ADD COLUMN IF NOT EXISTS bio TEXT";
        statement.execute(alterUserTableAddBio);

    }
    
    /*
     * Method for staff to staff/instructor messaging and notification. Responsible
     * for sending the initial alert, getting staff alerts, getting unresolved flags,
     * marking flags as resolved, and checking if flags are resolved.
     */
    
    // Method for checking if flags are resolved
    public boolean isFlagResolved(String itemId, Flag.FlagType type) {
        try {
            ensureConnected();
            String query = "SELECT resolved FROM Flags WHERE itemId = ? AND type = ?";
        ...
        return false; // Default to unresolved if error
    }
    
    // Method for marking flags as resolved
    public void markFlagAsResolved(String itemId, Flag.FlagType type) {
        try {
            ensureConnected();
            String sql = "UPDATE Flags SET resolved = TRUE WHERE itemId = ? AND type = ?";
    ...
    // Method for getting unresolved flags
    public int getUnresolvedFlagCountByUser(String username) {
        int count = 0;
        String query = "SELECT COUNT(*) FROM Flags WHERE flaggedBy = ? AND resolved = FALSE";
    ...
    // method for sending message to staff and instructors
    public void sendInternalAlert(String sender, String message, String itemId, Flag.FlagType type) throws SQLException {
        ensureConnected();

        String content = "[Internal Alert] Related to " + type + " ID " + itemId + ":\n\n" + message;

        // Use a placeholder recipient since alerts are intended for all staff/instructors
        String recipient = "ALL_STAFF_INSTRUCTORS"; 

        String sql = "INSERT INTO messages (sender, recipient, question_id, answer_id, content, message_type, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)";

        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, sender);
            stmt.setString(2, recipient);

            // Use type to determine whether it's related to a question, answer, or message
    ...
    // Method for geting staff alerts for display
    public List<Message> getInternalStaffAlerts(String currentUser) throws SQLException {
        ensureConnected();

        List<Message> alerts = new ArrayList<>();
        String sql = "SELECT * FROM messages WHERE message_type = 'staff_alert' " +
                "AND (recipient = 'ALL_STAFF_INSTRUCTORS' OR recipient = ?) " +
    ...
    // helper method for getting users by role
    public List<String> getUsersByRole(String role) {
        List<String> users = new ArrayList<>();
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             PreparedStatement stmt = conn.prepareStatement("SELECT userName FROM user_roles WHERE role = ?")) {
    ...
    /*
     * Methods for staff flagging functionality. Responsible for debugging, retrieving messages, 
     * clearing flags, retrieving flags, and inserting flags.
     */
    
    // DEBUG METHOD FOR MESSAGES
    public void printAllMessages() {
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT * FROM Messages")) {

            System.out.println("All messages in the database:");
            while (rs.next()) {
                System.out.println("ID: " + rs.getInt("id")
                    + ", From: " + rs.getString("sender")
                    + ", To: " + rs.getString("recipient")
                    + ", QID: " + rs.getString("question_id")
                    + ", AID: " + rs.getString("answer_id")
    ...
    // DEBUG METHOD FOR STAFF ALERTS
    public void debugPrintAllStaffAlerts() {
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             PreparedStatement stmt = conn.prepareStatement("SELECT * FROM messages WHERE message_type = 'staff_alert'");
             ResultSet rs = stmt.executeQuery()) {

            System.out.println("📬 All staff_alert messages in DB:");
            while (rs.next()) {
                System.out.println("ID: " + rs.getInt("id") +
                                   ", From: " + rs.getString("sender") +
                                   ", To: " + rs.getString("recipient") +
    ...
    // method for retrieving message from ID (used for staff viewing in dashboard)
    ...
    // method for getting message thread (used for staff viewing and flagging)
    public List<Message> getMessagesForQuestion(int questionId) {
        List<Message> messages = new ArrayList<>();

        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
            PreparedStatement stmt = conn.prepareStatement("SELECT * FROM Messages WHERE question_id = ? ORDER BY timestamp ASC")) {
            ...
            while (rs.next()) {
                ...
                LocalDateTime timestamp = LocalDateTime.parse(rs.getString("timestamp").replace(' ', 'T'));
                ...
                Message msg = new Message(
                	    ...
                	    recipient,
                	    qId,
                	    -1,              // no answerId for question messages
                	    content,
                	    Timestamp.valueOf(timestamp),
                	    false,           // default since read/unread isn't needed
                	    "question"       // default message type for this use case
                );
    ...
    // clear all flags
    public void clearAllFlags() throws SQLException {
        ensureConnected();
        String sql = "DELETE FROM Flags";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.executeUpdate();
            System.out.println("Flags table reset successfully.");
        }

        String resetAutoIncrement = "ALTER TABLE Flags ALTER COLUMN id RESTART WITH 1";
        try (PreparedStatement resetStmt = connection.prepareStatement(resetAutoIncrement)) {
            resetStmt.executeUpdate();
        }
    }
    
    // Method for deleting flags when resolved
    public void deleteFlag(String itemId, Flag.FlagType type) {
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             PreparedStatement stmt = conn.prepareStatement("DELETE FROM Flags WHERE itemId = ? AND type = ?")) {
            stmt.setString(1, itemId);
            stmt.setString(2, type.toString());
            stmt.executeUpdate();
        } catch (SQLException e) {
    ...
    // Method for getting flags
    public List<Flag> getAllFlags() {
        List<Flag> flags = new ArrayList<>();
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT * FROM Flags ORDER BY timestamp DESC")) {

            while (rs.next()) {
                Flag.FlagType type = Flag.FlagType.valueOf(rs.getString("type"));
                String itemId = rs.getString("itemId");
                String flaggedBy = rs.getString("flaggedBy");
                String reason = rs.getString("reason");
                LocalDateTime timestamp = LocalDateTime.parse(rs.getString("timestamp").replace(' ', 'T'));

                flags.add(new Flag(type, itemId, flaggedBy, reason, timestamp));
            }
    ...
    // Method for inserting flags
    public static void insertFlag(Flag flag) {
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
             PreparedStatement pstmt = conn.prepareStatement(
                 "INSERT INTO Flags (type, itemId, flaggedBy, reason, timestamp, resolved) VALUES (?, ?, ?, ?, ?, ?)"
             )) {
            pstmt.setString(1, flag.getType().toString());
            pstmt.setString(2, flag.getItemId());
            pstmt.setString(3, flag.getFlaggedBy());
            pstmt.setString(4, flag.getReason());
            pstmt.setTimestamp(5, Timestamp.valueOf(flag.getTimestamp()));
    ...
    /*
     * Methods for reviewer and reviews functionality. Responsible for clearing notifications, 
     * updating reviewer list, retrieving review history, clearing reviewer role request, 
     * adding reviews, and feedback messages for reviews.
     */
 ...
 // Clears the reviewer request for a user (removes the request entirely)
    ...
    /*
     * Methods for messages. Responsible for getting messages for a user, between users,
     * sending, retrieving, and clearing messages. 
     */
    ...
    /*
     * Methods for saving, updating, deleting, and retrieving questions 
    */
    ...
    /*
     * Methods for saving, updating, and loading answers and threaded replies.
     */
    ...
    public void storeInvitation(String code, String roles, LocalDate date, String time) throws SQLException {
        ...
        // Normalize the time string if it doesn't include seconds
        if (time.matches("^\\d{2}:\\d{2}$")) {
            time += ":00";
        }

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss");
        LocalDateTime expirationDateTime = LocalDateTime.parse(date + "T" + time, formatter);
	...
	public Answer getAnswerById(int answerId) throws SQLException {
	  ...
	  /**
	   * Updates the bio for a given user.
	   *
	   * @param username The username of the user whose bio is to be updated.
	   * @param bio      The new bio content.
	   * @throws SQLException If a database error occurs.
	   */
	  public void updateBio(String username, String bio) throws SQLException {
	      ensureConnected();
	      String sql = "UPDATE cse360users SET bio = ? WHERE userName = ?";
	      try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
	          pstmt.setString(1, bio);
	          pstmt.setString(2, username);
	          pstmt.executeUpdate();
	          System.out.println("Bio updated successfully for user: " + username); 
	      } catch (SQLException e) {
	          System.err.println("Error updating bio for user " + username + ": " + e.getMessage());
	          throw e;
	      }
	  }
	  
	  /**
	   * Retrieves the bio for a given user.
	   *
	   * @param username The username of the user.
	   * @return The user's bio, or null if not found or not set.
	   * @throws SQLException If a database error occurs.
	   */
	  public String getBio(String username) throws SQLException {
	      ensureConnected();
	      String sql = "SELECT bio FROM cse360users WHERE userName = ?";
	      try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
	          pstmt.setString(1, username);
	          try (ResultSet rs = pstmt.executeQuery()) {
	              if (rs.next()) {
	          ...
	          System.err.println("Error retrieving bio for user " + username + ": " + e.getMessage());
	          throw e;
	      }
	      return null; 
	  }
	  ...
	  /**
	   * Calculates the average rating given by a reviewer based on their latest reviews.
	   *
	   * @param reviewerUsername The username of the reviewer.
	   * @return The average rating as a double, or 0.0 if the reviewer has no latest reviews.
	   * @throws SQLException If a database error occurs.
	   */
	  public double getAverageRatingForReviewer(String reviewerUsername) throws SQLException {
		    ensureConnected();
		    String sql = "SELECT AVG(rating) FROM reviews WHERE reviewer_username = ? AND is_latest = TRUE";
		    try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
		        pstmt.setString(1, reviewerUsername);
		        ResultSet rs = pstmt.executeQuery();
		...
		public List<Message> getFeedbackMessagesForReviewer(String reviewer) throws SQLException {
		    ensureConnected();
		    String sql = "SELECT * FROM messages WHERE recipient = ? AND message_type = 'review-feedback'";
		    try (PreparedStatement stmt = connection.prepareStatement(sql)) {
		        stmt.setString(1, reviewer);
		        ResultSet rs = stmt.executeQuery();
		        List<Message> messages = new ArrayList<>();

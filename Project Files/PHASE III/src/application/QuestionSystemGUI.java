...
public class QuestionSystemGUI extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        topBar.getChildren().addAll(searchLabel, searchTextField, searchButton, spacer);

        if ("student".equalsIgnoreCase(activeRole)) {
            topBar.getChildren().add(trustedReviewersBtn); // adds trusted reviewer button if student role selected
        }
    ...
    private void showQuestionDetailInPanel(Question question, VBox questionDetailPanel) {
        ...
        questionDetailPanel.getChildren().addAll(
            contentArea, tagBox, answersLabel, answersListView,
    ...
    private VBox createAnswerBox(Answer answer, int questionId) {
        ...
        // box for reply, edit, and mark as accepted button
        HBox actionBox = new HBox(5, replyButton, editButton, markAsAcceptedButton);
    ...
    private void filterQuestions(String category) {
        if (category.equals("All")) {
            ...
            try {
                List<Question> filtered = databaseHelper.getQuestionsByTag(category);
